...
public class SpringContextTestCase {
	...
	public void assertComponentsInContext(String basePackage){
		...
		if(scannedComponents.size() > 0){
			throw new MissingComponentException("There are beans marked with '@Component' in the classpath that are not configured by Spring. " +
					"Either configure these beans or ignore them with the 'ignoreClassNames' or 'ignorePackages' method.\n" +
					"Components not in Spring:" + missingComponentList(scannedComponents));
		}
	}
